...
## PAM data file to read in
  pathway <- file.path(dir_input, "Transplant_PAM_15Jun2015_edit.csv")

## Reformat the data from the PAM
  PAMdf15 <- ParsePAM_KBG(pathway)
  str(PAMdf15)

## Drop bad Memory lines based on redos in notes
  mem.drop15 <- c(214, 272, 339, 416, 474, 492, 609, 618, 627, 636, 782, 838)

  #PAMdf15 <- subset(PAMdf15[[1]], !(Mem %in% mem.drop15))
  PAMdf15 <- lapply(PAMdf15,
                    function(x) subset(x, !(Mem %in% mem.drop15)))


## Extract regression parameters and make a separate dataframe
  rp <- as.data.frame(PAMdf15[1])
...
## Experimental metadata file to read in
  info <- read.csv(file.path(dir_input, "Metadata_15Jun2015_edit.csv"))
...
#
#   lc.plot1 <- ggplot(data=lc15[which(lc15$Site == 4 & lc15$Algae == "Peri"),], aes(x=PAR, y=ETR, group = Mem))
#
#   lc.plot1 + geom_line(aes(color=ArrayID), size = 0.75) + geom_point(aes(color=ArrayID),size = 4) + labs(x=expression(paste("PAR ",mu,"Mols ",m^{-2}," ", s^{-1} )), y="ETR") + facet_grid(Treatment~.) + ggtitle("Rapid Light Curve") + theme_bw(base_size=20)
#
...
# ## Plot Fv/Fm
#
#   FvFm.plot<-ggplot(data=lc15, aes(x = Site, y = Fv.Fm, group = Mem))
#
