## This script formats data from the PAM flurometer from the transplant experiment in June 2015
...
## PAM data file to read in
  pathway <- file.path(dir_input, "Transplant_PAM_11Jun2015_edit.csv")

## Reformat the data from the PAM
  PAMdf11 <- ParsePAM_KBG(pathway)

## Drop bad Memory lines based on redos in notes
  mem.drop11 <- c(72, 125, 192, 621, 735)

  PAMdf11 <- lapply(PAMdf11,
                    function(x) subset(x, !(Mem %in% mem.drop11)))

## Extract regression parameters and make a separate dataframe
  rp <- as.data.frame(PAMdf11[1])
...
## Experimental metadata file to read in
  info <- read.csv(file.path(dir_input, "Metadata_11Jun2015_edit.csv"))
...
#
#   lc.plot1 <- ggplot(data=lc11[which(lc11$Site == 3 & lc11$Algae == "Oed"),], aes(x=PAR, y=ETR, group = Mem))
#
...
# ## Plot Fv/Fm
#
#   FvFm.plot<-ggplot(data=lc11, aes(x = Site, y = Fv.Fm, group = Mem))
#
