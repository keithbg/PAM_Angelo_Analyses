...
##### Read in the data #####
  dir_input <- file.path("/Users", "kbg", "Dropbox", "PAM_Angelo", "PAM_Angelo_Analyses", "2015", "PAM_data", "PAM_data_raw")


## PAM data file to read in
  pathway <- file.path(dir_input, "Transplant_PAM_09Jun2015_edit.csv")

## Reformat the data from the PAM
  PAMdf09 <- ParsePAM_KBG(pathway)
  str(PAMdf09)

## Drop bad Memory lines based on redos in notes
  mem.drop09 <- c(151, 196, 476, 504, 561, 573, 750, 777)
  PAMdf09 <- lapply(PAMdf09,
                    function(x) subset(x, !(Mem %in% mem.drop09)))

## Extract regression parameters and make a separate dataframe
  rp <- as.data.frame(PAMdf09[1])
...
## Experimental metadata file to read in
  info <- read.csv(file.path(dir_input, "Metadata_09Jun2015_edit.csv"))
...
#   lc.plot1 <- ggplot(data=lc09[which(lc09$Site == 4 & lc09$Algae == "Oed"),], aes(x=PAR, y=ETR, group = Mem))
#
...
# ## Plot Fv/Fm
#
#   FvFm.plot<-ggplot(data=lc, aes(x = Site, y = Fv.Fm, group = Mem))
#
